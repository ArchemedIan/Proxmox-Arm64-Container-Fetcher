...
fixTarball () {
	...
	if [ "$1" = "debian" ] ; then
		...
		pct exec 999999999 -- DEBIAN_FRONTEND=noninteractive apt install -yq ifupdown wget -y >/dev/null 2>&1
  		[[ $2 = 12 ]] && pct exec 999999999 -- DEBIAN_FRONTEND=noninteractive apt install -yq apt-listchanges apt-utils bash-completion bind9-dnsutils bind9-host bind9-libs bsdextrautils bzip2 cpio cron-daemon-common cron debconf-i18n debian-faq distro-info-data dmidecode doc-debian fdisk file gettext-base groff-base inetutils-telnet kmod less libbrotli1 libcurl3-gnutls libfstrm0 libicu72 libjansson4 libjemalloc2 libldap-2.5-0 liblmdb0 liblocale-gettext-perl liblockfile-bin libmagic-mgc libmagic1 libmaxminddb0 libnewt0.52 libnftables1 libnftnl11 libnghttp2-14 libnsl2 libpipeline1 libpopt0 libprotobuf-c1 libpython3-stdlib libpython3.11-minimal libpython3.11-stdlib libreadline8 librtmp1 libsasl2-2 libsasl2-modules-db libslang2 libsqlite3-0 libssh2-1 libtext-charwidth-perl libtext-iconv-perl libtext-wrapi18n-perl libuchardet0 libuv1 libwrap0 libxml2 logrotate lsof mailcap man-db manpages media-types mime-support nano ncurses-term netcat-traditional nftables openssh-server openssh-sftp-server postfix python-apt-common python3-apt python3-certifi python3-chardet python3-charset-normalizer python3-debconf python3-debian python3-debianbts python3-httplib2 python3-idna python3-minimal python3-pkg-resources python3-pycurl python3-pyparsing python3-pysimplesoap python3-reportbug python3-requests python3-six python3-urllib3 python3.11-minimal python3.11 python3 readline-common reportbug runit-helper sensible-utils ssh ssl-cert systemd-timesyncd tasksel-data tasksel traceroute ucf vim-tiny wamerican whiptail xz-utils >/dev/null 2>&1
    		[[ ! $2 = 12 ]] && pct exec 999999999 -- DEBIAN_FRONTEND=noninteractive apt install -yq python3 python3-dev python3-pip python3-venv
		...
		pct exec 999999999 -- apt update >/dev/null 2>&1
		pct exec 999999999 -- apt install wget ifupdown python3 python3-dev python3-pip python3-venv -y >/dev/null 2>&1
		pct exec 999999999 -- mv /etc/systemd/network/eth0.{network,off} >/dev/null 2>&1
		pct stop 999999999 >/dev/null 2>&1
		pct unmount 999999999 >/dev/null 2>&1
  		[[ "$quiet" -gt 4 ]] && msg_ok "Updated Image And Applied Fix(es)"
